...
def render_design_export_md(
    ...
    *,
    ...
    lines.append(">")
    lines.append("> **이 문서를 받은 외부 AI에게**:")
    lines.append("> 사용자가 What·Why를 거쳐 여기에 도착했다는 컨텍스트를 인지한 상태에서,")
    lines.append("> 사용자가 요청한 How(구현·작성)를 진행해주세요.")
    lines.append("> 사용자가 *\"답을 적어둔 상태\"* 가 아니라 *\"질문을 인지한 상태\"* 일 수 있으니,")
    lines.append("> 답 디테일이 필요하면 사용자에게 직접 보충 질문하세요.")
    lines.append("")
    lines.append("---")
    lines.append("")
