...
class NaverOAuthProvider(OAuthProviderClient):
    name = OAuthProvider.NAVER.value
    label = "Naver"
    ...
    def exchange_code(self, code: str, state: str | None = None) -> OAuthTokens:
        data = self._post_form(
            NAVER_TOKEN_URL,
            {
                ...
                "code": code,
                # 네이버는 토큰 교환 시 state 파라미터를 요구한다.
                # callback에서 받은 원본 state를 그대로 전달하면 안전하고,
                # 미전달 시에는 랜덤 값을 보낸다 (별도 검증을 하지 않음).
                "state": state or secrets.token_urlsafe(16),
            },
            action="토큰 교환",
        )
    ...
    def get_user_info(self, access_token: str) -> OAuthUserInfo:
        data = self._get_with_token(
            NAVER_USERINFO_URL, access_token, action="사용자 정보 조회"
        )
