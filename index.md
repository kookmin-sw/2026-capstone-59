...
| **의사결정 궤적 추출** | 어떤 구간(Stage / 필수 Step)을 .md로 추출하는지, 추출 후 어떤 외부 AI(Claude · ChatGPT · Cursor · Codex 등)와 연결되는지 |
...
- **외부 AI 연결 패턴 분석** — 의사결정 궤적 .md가 어떤 구간을 가장 많이 추출하는지, 추출 후 어떤 외부 AI와 어떤 작업으로 이어지는지가 데이터로 남으면, *"What · Why → How"* 사이의 사용자 워크플로우를 정량적으로 이해할 수 있다.
